 ...
 *
 * UPDATE: passes `navigate` to showNotificationToast so clicking
 * a toast navigates directly to the relevant page.
 */
...
export function useNotificationSocket() {
  ...
  const navigate = useNavigate(); // ← needed so toasts can navigate
  ...
  useEffect(() => {
    ...
    socket.on("notification:new", (notification) => {
      ...
      // 2. Show toast — pass navigate so clicking the toast goes to the right page
